...
const days = computed(() =>
  selectedGoal.value ? daysByGoal[selectedGoal.value] : [],
);
function openGoal(goal: TrainingGoal) {
...
const currentDay = computed(() =>
  days.value.find((day) => day.id === selectedDay.value),
);
...
const done = computed(
  () =>
    currentDay.value?.exercises.reduce(
      (sum, ex) =>
        sum +
...
<template>
  <div class="app-shell">
    ...
    <main v-if="selectedGoal === null" class="home container goal-home">
      <span class="eyebrow"
        ><span class="eyebrow-dot"></span> ELIGE TU OBJETIVO</span
      >
      <h1>Un plan para <em>tu objetivo.</em></h1>
      <p class="intro">
        Selecciona primero qué quieres trabajar. Cada objetivo tiene tres
        sesiones por semana, imágenes de apoyo, demostraciones y seguimiento
        temporal de series.
      </p>
      <div class="goal-grid">
        <button
        ...
        >
          <span class="goal-tag">{{ goal.subtitle }}</span>
          <h2>{{ goal.title }}</h2>
          <p>{{ goal.description }}</p>
          <span class="goal-link">Ver mis 3 días →</span>
        </button>
      </div>
      <p class="footnote">
        Sin cuentas ni persistencia: las marcas se reinician al recargar. Las
        ilustraciones son orientativas; los tutoriales externos requieren
        internet.
      </p>
    </main>
    ...
    <main v-else-if="selectedDay === null" class="home container">
      ...
      >
      <h1>
        {{ goals.find((goal) => goal.id === selectedGoal)?.title }}<br /><em
          >3 días por semana.</em
        >
      </h1>
      <p class="intro">
        {{ goals.find((goal) => goal.id === selectedGoal)?.description }} Elige
        tu día, consulta una demostración y marca las series completadas.
      </p>
      <div class="home-info">
    ...
    <main v-else-if="currentDay" class="workout container">
      <button
        ...
        "
      >
        ← Días de {{ goals.find((goal) => goal.id === selectedGoal)?.title }}
      </button>
      <div class="workout-heading">
        <div>
          <span class="eyebrow"
            >{{ goals.find((goal) => goal.id === selectedGoal)?.title }} · DÍA
            {{ String(currentDay.id).padStart(2, "0") }} / ENTRENAMIENTO</span
          >
    ...
    <footer class="site-footer container">
      <span>MI GYM / 2 OBJETIVOS · 3 DÍAS</span
      ><span
